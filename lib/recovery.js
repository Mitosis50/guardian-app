...
const { downloadTransaction } = require('./arweave-gateway')
...
/**
 * Recover a single file by CID or Arweave tx id.
 *
 * @param {object} opts
 * @param {string} [opts.cid]           IPFS CID
 * @param {string} [opts.arweaveTxId]   Arweave transaction id
 ...
 */
async function recoverOne({ cid, arweaveTxId, filename, keyPath, outputDir, onProgress }) {
  const emit = onProgress || (() => {})

  if (!cid && !arweaveTxId) {
    throw new Error('Either cid or arweaveTxId is required to recover a file.')
  }
  ...
  try {
    if (arweaveTxId) {
      emit(`↓ Downloading ${cleanName} from Arweave…`)
      const bytes = await downloadTransaction(arweaveTxId)
      await fs.writeFile(tempEnc, bytes)
    } else {
      emit(`↓ Downloading ${cleanName} from IPFS…`)
  ...
  for (const [filename, record] of latest.entries()) {
    try {
      const outPath = await recoverOne({
        cid: record.cid,
        arweaveTxId: record.arweave_tx_id || record.arweaveTxId,
        filename,
        keyPath,
        outputDir,
        onProgress: emit,
