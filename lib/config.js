...
function saveConfig(store, partial) {
  ...
  next.arweaveEnabled = Boolean(next.arweaveEnabled)
  next.arweaveWalletPath = String(next.arweaveWalletPath || '').trim()
  next.arweaveHost = String(next.arweaveHost || DEFAULTS.arweaveHost).trim()
  next.arweavePort = Number(next.arweavePort || DEFAULTS.arweavePort)
  next.arweaveProtocol = String(next.arweaveProtocol || DEFAULTS.arweaveProtocol).trim()
  next.arweaveTimeoutMs = Number(next.arweaveTimeoutMs || DEFAULTS.arweaveTimeoutMs)
  store.set(next)
}
